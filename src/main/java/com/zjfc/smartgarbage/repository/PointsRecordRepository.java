...
public interface PointsRecordRepository extends JpaRepository<PointsRecord, Long> {
        ...
        // 根据用户ID和变动类型查询积分记录 - 字段名改为 changeType
        ...
        // 根据用户ID和时间段查询积分记录
        List<PointsRecord> findByUserIdAndCreatedAtBetween(String userId, LocalDateTime startTime,
        ...
        // 按变动类型统计积分 - 修正：pointsType 改为 changeType
        ...
        // 新增：查询今日积分
        @Query("SELECT pr FROM PointsRecord pr WHERE pr.userId = :userId AND DATE(pr.createdAt) = CURRENT_DATE")
        List<PointsRecord> findTodayPoints(@Param("userId") String userId);
}